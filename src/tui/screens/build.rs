...
use crate::tui::theme::{self, jp, display_width};
...
fn render_progress(screen: &BuildScreen, area: Rect, buf: &mut Buffer) {
    ...
    let header_x = area.x + (area.width.saturating_sub(display_width(&header_line))) / 2;
    ...
    let max_line_width = log_area.width.saturating_sub(4) as usize;
    for (i, line) in screen.log_lines.iter().rev().take(log_area.height.saturating_sub(2) as usize).enumerate() {
        ...
        let display_line = truncate_to_width(line, max_line_width);
        buf.set_string(log_area.x + 2, y, &display_line, theme::code());
    }

    // Help
    let help = "Building... Press [Q] to cancel";
    let help_x = area.x + (area.width.saturating_sub(display_width(help))) / 2;
...
fn render_complete(screen: &BuildScreen, area: Rect, buf: &mut Buffer) {
    ...
    let title_x = banner_x + (banner_width.saturating_sub(display_width(&title))) / 2;
    buf.set_string(
        title_x,
        banner_area.y + 1,
        &title,
    ...
    let jp_x = banner_x + (banner_width.saturating_sub(display_width(jp_title))) / 2;
    ...
    let prompt_x = area.x + (area.width.saturating_sub(display_width(prompt))) / 2;
