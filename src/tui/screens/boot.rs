...
use crate::tui::theme::{self, jp, display_width, BANNER_LINES, BANNER_WIDTH};
...
impl Widget for &BootScreen {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let subtitle_x = area.x + (area.width.saturating_sub(display_width(&subtitle))) / 2;
        buf.set_string(subtitle_x, chunks[2].y, &subtitle, theme::kanji());

        // Status line
        let status = format!("INITIALIZING SYSTEM //{}", jp::SYSTEM_BOOT);
        let status_x = area.x + (area.width.saturating_sub(display_width(&status))) / 2;
