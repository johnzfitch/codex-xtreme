...
impl Widget for Panel<'_> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        if let Some(title) = self.title {
            ...
            let decorated_width = theme::display_width(&decorated);
            if title_x + decorated_width <= area.x + area.width - 2 {
                buf.set_string(title_x, area.y, &decorated, title_style);
            }

            // Japanese subtitle
            if let Some(jp) = self.title_jp {
                let jp_str = format!("//{}", jp);
                let jp_x = title_x + decorated_width + 1;
                if jp_x + theme::display_width(&jp_str) <= area.x + area.width - 2 {
                    buf.set_string(jp_x, area.y, &jp_str, theme::kanji());
                }
            }
        }
    }
